...
# Fetch Ethereum prices from January 1, 2022 to present
print("Starting Ethereum price data fetch...")
eth_df = fetch_historical_prices('ETHUSDT', '2022-01-01')

# Print summary
print(f"\nFetched {len(eth_df)} days of Ethereum price data")
if not eth_df.empty:
    print(f"Date range: {eth_df['Date'].iloc[0]} to {eth_df['Date'].iloc[-1]}")
    print(f"\nFirst few rows:")
    print(eth_df.head())
    print(f"\nLast few rows:")
    print(eth_df.tail())
...
# Export to CSV
output_file = "eth_prices_2022_present.csv"
eth_df.to_csv(output_file, index=False)
