...
# Fetch XRP prices from January 1, 2022 to present
print("Starting XRP price data fetch...")
xrp_df = fetch_historical_prices('XRPUSDT', '2022-01-01')

# Print summary
print(f"\nFetched {len(xrp_df)} days of XRP price data")
if not xrp_df.empty:
    print(f"Date range: {xrp_df['Date'].iloc[0]} to {xrp_df['Date'].iloc[-1]}")
    print(f"\nFirst few rows:")
    print(xrp_df.head())
    print(f"\nLast few rows:")
    print(xrp_df.tail())
...
# Export to CSV
output_file = "xrp_prices_2022_present.csv"
xrp_df.to_csv(output_file, index=False)
